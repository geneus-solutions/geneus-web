...
const CourseDescription = ({courseDetails}) => {
    ...
    useEffect(() => {
        ...
        const saveVisitorData = async (ip, city) => {
            try {
                await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/visitors`, {
                    ip,
                    city
                }, {
                    headers: {
    ...
    useEffect(() => {
        ...
        if (courseDetails && courseDetails?.aboutCourse?.details) {
            setFormattedAboutCourse(
                courseDetails &&
                courseDetails.aboutCourse.details.map((detail) =>
                    formatText(detail)
                )
            );
        }
                    ...
                    <MDBCardBody className="pb-2">
                        ...
                        <div className="pt-3">
                            ...
                            {courseDetails && courseDetails?.mentorImage && (
                                <img src={courseDetails?.mentorImage} alt="Mentor" className="mentor-image" />
                            )}
                        </div>
                    </MDBCardBody>
                </MDBCol>
                ...
                <MDBCol md="12" lg="6">
                    ...
                    <MDBCardText className="fs-6 fw-normal requirements-text">
                        <div className="div-margin">
                            <ul>
                                {courseDetails &&
                                    courseDetails?.requirements?.map(
                                        (requirement, index) => (
                                            <li key={index}>
                                                {requirement}
                                            </li>
