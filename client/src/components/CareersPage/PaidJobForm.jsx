...
const OpportunityForm = () => {
  const [postPaidJob, { isLoading, isError, isSuccess, error }] = usePostpaidjobMutation();
  ...
  const getTypeIcon = (type) => {
    switch(type) {
      case 'job': return <FaBriefcase className="w-5 h-5" />;
      case 'course': return <FaGraduationCap className="w-5 h-5" />;
      case 'internship': return <FaUsers className="w-5 h-5" />;
      default: return null;
    }
  };
  ...
  const getLocationIcon = (locationType) => {
    switch(locationType) {
      case 'remote': return <FaGlobe className="w-4 h-4" />;
      case 'onsite': return <FaBuilding className="w-4 h-4" />;
      case 'hybrid': return <FaHome className="w-4 h-4" />;
      default: return null;
    }
  };
...
const prepareFormData = () => {
  ...
  const filteredResponsibilities = formData.jobDetails.responsibilities.filter(resp => resp.trim() !== '');
  const filteredRequirements = formData.jobDetails.requirements.filter(req => req.trim() !== '');
  const filteredModules = formData.courseDetails.modules.filter(module => module.trim() !== '');
  ...
  if (formData.type === 'job') {
    submissionData.jobDetails = {
      salary: formData.jobDetails.salary ? Number(formData.jobDetails.salary) : undefined,
    ...
    submissionData.courseDetails = {
      price: formData.courseDetails.price ? Number(formData.courseDetails.price) : undefined,
      durationWeeks: formData.courseDetails.durationWeeks ? Number(formData.courseDetails.durationWeeks) : undefined,
    ...
    submissionData.internshipDetails = {
      ...
      stipendAmount: formData.internshipDetails.stipendAmount ? Number(formData.internshipDetails.stipendAmount) : undefined,
      stipendRange: (formData.internshipDetails.stipendRange.min && formData.internshipDetails.stipendRange.max) 
        ? {
            min: Number(formData.internshipDetails.stipendRange.min),
            max: Number(formData.internshipDetails.stipendRange.max)
          }
        : undefined,
      lastDateToApply: formData.internshipDetails.lastDateToApply || undefined
    };
  ...
  console.log('Data being sent to backend:', JSON.stringify(submissionData, null, 2));
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-8 text-white">
            ...
            <h5 className="text-xl text-white mb-2 ml-52">Fill in the details to post a new opportunity</h5>
          </div>
          ...
          {isError && (
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded m-4">
              {error?.data?.message || 'Failed to create opportunity. Please try again.'}
            </div>
          )}
                  ...
                  <div className="flex gap-4">
                    {['public', 'private'].map(vis => (
                      <button
                        key={vis}
                        type="button"
                        onClick={() => setFormData(prev => ({ ...prev, visibility: vis }))}
                        disabled={isLoading}
                        className={`
                        ...
                        `}
                      >
                        {vis === 'public' ? <FaEye className="w-4 h-4" /> : <FaEyeSlash className="w-4 h-4" />}
                        {vis}
                      </button>
                    ))}
                  </div>
                    ...
                    <div key={index} className="flex gap-2 mb-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          type="text"
                          value={resp}
                          onChange={(e) => handleArrayChange('responsibilities', index, e.target.value, 'jobDetails')}
                          className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      ...
                      {index > 0 && (
                        <button
                          type="button"
                          onClick={() => removeField('responsibilities', index, 'jobDetails')}
                    ...
                    <div key={index} className="flex gap-2 mb-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          type="text"
                          value={req}
                          onChange={(e) => handleArrayChange('requirements', index, e.target.value, 'jobDetails')}
                          className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      ...
                      {index > 0 && (
                        <button
                          type="button"
                          onClick={() => removeField('requirements', index, 'jobDetails')}
                          className="w-10 h-10 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors flex items-center justify-center"
                        >
                          <FaTimes className="w-4 h-4" />
                        </button>
                    ...
                    <div key={index} className="flex gap-2 mb-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          type="text"
                          value={module}
                          onChange={(e) => handleArrayChange('modules', index, e.target.value, 'courseDetails')}
                          className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                      ...
                      {index > 0 && (
                        <button
                          type="button"
                          onClick={() => removeField('modules', index, 'courseDetails')}
                          className="w-10 h-10 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors flex items-center justify-center"
                        >
                          <FaTimes className="w-4 h-4" />
                        </button>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={(e) => handleNestedInputChange('internshipDetails', 'stipendRange', 'min', e.target.value)}
                          className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={(e) => handleNestedInputChange('internshipDetails', 'stipendRange', 'max', e.target.value)}
                          className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
